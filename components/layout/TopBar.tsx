...
export function TopBar({ userName = 'BRE', userLevel = 1, avatarUrl, isGuest }: TopBarProps) {
  return (
    <header className="sticky top-0 z-40 bg-bg/80 backdrop-blur-md border-b border-ash-200/60">
        ...
        <div className="hidden lg:flex flex-1 max-w-md">
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full h-10 pl-10 pr-4 rounded-full bg-surface border border-ash-200 text-body-sm placeholder:text-ash-400 focus:outline-none focus:border-honey-500 focus:ring-2 focus:ring-honey-200 transition"
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 lg:gap-3">
          {!isGuest ? (
            <Link href="/slots/new" className="hidden lg:inline-flex">
              <span className="inline-flex items-center justify-center gap-2 h-9 px-3.5 text-body-sm font-medium rounded-2xl bg-honey-500 text-graphite shadow-honey hover:bg-honey-400 transition">
          ...
          >
            ...
            <span className="absolute top-2 right-2 size-2 rounded-full bg-honey-500 ring-2 ring-bg" />
          </Link>
