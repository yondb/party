...
export function SlotCreateForm({ places, initialPlaceId }: SlotCreateFormProps) {
  ...
  const [place, setPlace] = useState<PlaceRow | null>(
    () => places.find((p) => p.id === initialPlaceId) ?? null,
  );
    ...
    <div>
      ...
      <div className="mb-6 flex gap-2">
        {[1, 2, 3].map((n) => (
          <div
            key={n}
            className={`h-1 flex-1 rounded-full ${step >= n ? "bg-[var(--gold-mid)]" : "bg-[var(--gold-dim)]/40"}`}
          />
        ))}
      </div>

      <form id="slot-create-form" onSubmit={onSubmit} className="space-y-6 pb-28 md:pb-6">
        {step === 1 ? (
          <section>
        ...
        {step === 2 ? (
          <section className="space-y-4">
            ...
            {place && placeMeta ? (
              <div className="wow-card rounded-lg border border-[var(--gold-dim)] p-3">
        ...
        {step === 3 ? (
          <section className="wow-card space-y-3 rounded-lg p-4">
            <h2 className="font-display text-sm font-semibold uppercase tracking-[0.12em] text-[var(--text-muted)]">
              {t.stepConfirm}
            </h2>
            <p>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--gold-dim)] bg-[var(--bg-void)]/95 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] backdrop-blur-sm md:hidden">
        {step < 3 ? (
          ...
          <Button type="submit" form="slot-create-form" variant="primary" fullWidth disabled={loading}>
            {loading ? t.creating : t.create}
          </Button>
        )}
      </div>
