...
export function ShareSlotPanel({ slotId, lang }: Props) {
  const t = growthUi(lang);
  ...
  return (
    <section className="rounded-2xl border border-honey-200/80 bg-honey-50/50 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        {aiGenerated ? (
          <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-surface px-2 py-0.5 text-caption font-medium text-honey-800">
      ...
      {text ? (
        <p className="mt-3 whitespace-pre-wrap rounded-xl border border-ash-200/60 bg-surface p-3 text-body-sm text-ash-800">
          {text}
        </p>
      ) : null}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <button
          ...
          disabled={loading}
          className={cn(
            'inline-flex h-10 items-center justify-center gap-2 rounded-2xl border border-ash-200 bg-surface px-4 text-body-sm font-medium text-ash-800 transition hover:bg-ash-50 disabled:opacity-50',
