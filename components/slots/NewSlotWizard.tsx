...
const PlacePickerModal = dynamic(() => import('@/components/slots/PlacePickerModal').then((m) => m.PlacePickerModal),
  { ssr: false },
);
...
export function NewSlotWizard({ places, initialPlaceId }: NewSlotWizardProps) {
  ...
  const [category, setCategory] = useState<CategoryId | null>(initialPlace ? toCategoryId(initialPlace.category) : null,
  );
  ...
  async function submit() {
    ...
    if ('error' in res) {
      setError(res.error ?? 'Could not create slot.');
      setSubmitting(false);
      return;
    }
  ...
  return (<div className="mx-auto max-w-2xl px-4 lg:px-8 py-6 lg:py-10">
      <div className="flex items-center justify-between mb-8">
        ...
        <span className="font-mono text-body-sm text-ash-500">Step {step}/2</span>
      </div>
      ...
      {step === 1 && (<div className="space-y-8 animate-fade-up">
          <header>
            <h1 className="font-display text-display-xl text-ash-900">What are you doing?</h1>
            <p className="mt-2 text-body-lg text-ash-500">
              {categoryLocked
                ? 'This place has a fixed activity — pick a time in the next step.'
                : 'Pick an activity and a place.'}
            </p>
          </header>

          {categoryLocked && hasPlace ? (<div className="space-y-2">
              <span className="block text-caption text-ash-500">Place</span>
              <div className="flex items-center gap-3 rounded-3xl border border-ash-200 bg-surface p-4 shadow-sm">
                ...
                <div className="min-w-0">
                  ...
                  {lockedCategory ? (<p className="text-body-sm text-ash-500">
                      {lockedCategory.emoji} {categoryLabel(lockedCategory.id)}
                    </p>
                  ) : null}
                </div>
                <button
              ...
              <div className="relative">
                ...
                />
                {showSuggestions && suggestions.length > 0 ? (<ul className="absolute z-20 mt-1 max-h-64 w-full overflow-y-auto rounded-2xl border border-ash-200/70 bg-surface p-1 shadow-lg">
                    {suggestions.map((p) => {
                          ...
                          >
                            ...
                            <span className="min-w-0">
                              ...
                              <span className="block truncate text-caption text-ash-500">
                                {[categoryLabel(toCategoryId(p.category)), p.district, p.city].filter(Boolean).join(' · ')}
                              </span>
                            </span>
                          </button>
                        </li>
              ...
              {hasPlace ? (<p className="flex items-center gap-1.5 text-body-sm text-success">
                  <Check className="size-4" />
                  {customPoint ? 'Map point: ' : 'Selected: '}
          ...
          {categoryLocked && lockedCategory ? (<div
            ...
            >
              ...
              <span className="font-display text-heading-md text-ash-900">{categoryLabel(lockedCategory.id)}</span>
              <p className="text-center text-caption text-ash-500">
                Activity tied to this place
              </p>
            </div>
          ) : (<div className="space-y-2">
              <span className="block text-caption text-ash-500">Activity</span>
              <div className="grid grid-cols-3 gap-2">
                {CATEGORY_LIST.map((cat) => {
                  ...
                  return (<button
                      ...
                      className={cn('rounded-2xl py-2.5 px-1 flex flex-col items-center justify-center gap-1 transition-all duration-200',
                        'border-2 min-h-[72px]',
                        active
      ...
      {step === 2 && category && (<div className="space-y-8 animate-fade-up">
          <header>
            <h1 className="font-display text-display-xl text-ash-900">When and who?</h1>
            <p className="mt-2 text-body-lg text-ash-500">Slot details.</p>
          </header>
          ...
          <Input label="Date and time" type="datetime-local" value={startAt} onChange={(e) => setStartAt(e.target.value)} />

          <Stepper label="Party size (with host)" value={partySize} min={2} max={10} onChange={setPartySize} />

          <div className="space-y-1.5">
            <span className="block text-caption text-ash-500">Who can join</span>
            <div className="grid grid-cols-3 gap-2">
              {([
                  { id: 'all' as const, label: 'Everyone', icon: Users },
                  { id: 'female' as const, label: 'Women only', icon: UserIcon },
                  { id: 'male' as const, label: 'Men only', icon: UserIcon },
                ] as const
                ...
                return (<button
                    ...
                    className={cn('h-20 rounded-2xl flex flex-col items-center justify-center gap-1 border transition',
                      active
          ...
          <div className="space-y-3 rounded-2xl border border-ash-200 bg-surface p-4">
            <span className="block text-caption text-ash-500">Requirements (optional)</span>
            <Stepper label="Minimum level" value={minLevel} min={0} max={20} onChange={setMinLevel} />
            <div className="space-y-1.5">
              <span className="block text-caption text-ash-500">Minimum reliability</span>
              <div className="grid grid-cols-4 gap-2">
                {[
                  ...
                  return (<button
                      ...
                      className={cn('h-10 rounded-xl border text-body-sm font-medium transition',
                        active
                          ? 'bg-graphite text-surface border-graphite'
                          : 'bg-surface text-ash-700 border-ash-200 hover:bg-ash-50',
                      )}
          ...
          <Textarea
            ...
            placeholder="Anything good to know?"
            value={description}
