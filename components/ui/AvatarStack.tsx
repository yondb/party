...
export function AvatarStack({ avatars, max = 3, size = 'sm', className }: AvatarStackProps) {
  ...
  return (
    <div className={cn('flex items-center', className)}>
      {shown.map((a, i) => (
        <div key={i} className={cn(ringClass, 'rounded-full', i > 0 && '-ml-2')}>
          <Avatar src={a.src} name={a.name} size={size} />
        </div>
      ))}
      {overflow > 0 && (
        <div className={cn(boxClass, '-ml-2 rounded-full bg-ash-200 text-ash-700 flex items-center justify-center font-mono font-semibold', ringClass, overflowText)}>
          +{overflow}
        </div>
      )}
    </div>
